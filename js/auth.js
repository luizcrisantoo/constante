...
function senhaInput(id, autocomplete){
  return '<div style="position:relative">'
    +'<input id="'+id+'" type="password"'+(autocomplete?' autocomplete="'+autocomplete+'"':'')+' style="width:100%;padding-right:2.4rem">'
    +'<button type="button" data-action="ver-senha" data-alvo="'+id+'" aria-label="mostrar ou ocultar a senha" style="position:absolute;right:0.5rem;top:50%;transform:translateY(-50%);background:none;border:none;font-size:1.15rem;line-height:1;cursor:pointer;padding:0.2rem">👁</button>'
    +'</div>';
}
...
function traduzErroAuth(error){
  ...
  if(/should contain|one character of each|does not meet|weak.?password/i.test(m)) return 'A senha precisa ter no mínimo 8 caracteres, com letra maiúscula, minúscula e número.';
